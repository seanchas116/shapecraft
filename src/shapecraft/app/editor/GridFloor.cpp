...
void GridFloor::draw(const DrawEvent &event) {
    ...
    event.drawMethods->drawLine(_vao, transform, event.camera, 0.5, vec4(0.5, 0.5, 0.5, 1));
    ...
    event.drawMethods->drawLine(_yAxisVAO, transform, event.camera, 1, color0);
    event.drawMethods->drawLine(_zAxisVAO, transform, event.camera, 1, color1);
}
